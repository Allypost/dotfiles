...
local function setMaximized(client, s)
    if s then
        client.border_width = 0
        client.shape = gears.shape.rect
        if client.floating then
            setTitlebar(client, false)
        end
    else
...
mytextclock = wibox.widget.textclock(" <span>%d-%m-%Y</span>, <span>%X</span> ")
...
globalkeys = gears.table.join(
    ...
    awful.key({ modkey, altkey    }, "c", function () awful.spawn(terminal) end,
              {description = "open a terminal", group = "launcher"}),
    ...
    awful.key({ "Shift"           }, "Print", function() awful.spawn.with_shell("maim --window $(xdotool getactivewindow) | xclip -selection clipboard -t image/png && notify-send 'Window Screenshot' \"<b>Captured screenshot:</b>\\n<i>$(xdotool getactivewindow getwindowname)</i>\" -i showfoto --app-name Screenshot") end,
...
clientkeys = gears.table.join(
    ...
    -- Menubar
    awful.key({ modkey            }, "p",      function (c) setTitlebar(c, c.floating)       end,
